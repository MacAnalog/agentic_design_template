"""The OPEN lane: deck text in, run directory out; `make doctor` proves it with one resistor.

**This is the default lane, and what `lane:` absent in `harness.yaml` means** — `design/sim.py`
re-exports this module unless the key names another. It WAS `design/sim.py` up to template v2.07;
the split changed the file name, added `main()` (so `python -m design.sim` can dispatch here) and
edited two comments. No behaviour changed. The other shipped lane is `design/sim_bridge.py`
(`lane: bridge`), for a commercial kit that simulates on the EDA server.
...
# `doc/environment.md` pins, e.g. `{"PDK_LIB": "<the revision name, as the library spells it>"}`.
