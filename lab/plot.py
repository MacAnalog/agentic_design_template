...
def eye(t: np.ndarray, x: np.ndarray, data: Data, out: Path, *, title: str = "",
    ...
    for ax, filt, lab in ((axs[0], False, "unfiltered"), (axs[1], True, f"after ref Rx (BT4, {bw:g} x baud)")):
    ...
    for ax, y in zip(axs[0], ys):
        for lab in labels:
            pts = sorted((r[x], r[y]) for r in rows
                         if r.get(by) == lab and isinstance(r.get(y), (int, float)))
            if pts:
                ax.plot([p[0] for p in pts], [p[1] for p in pts], "o-", ms=3, label=str(lab))
        spec_band(ax, y)
        r = SPEC.get(y)
