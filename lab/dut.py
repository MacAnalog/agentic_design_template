...
instantiations turned out to be multi-bench, and `lab.metrics`, the `deck-rebuild` lint and
...
class Design:
    ...
    def benches(self) -> list[str]:
        """The benches `lab.metrics` builds a deck for (a design bound to a circuit database
    ...
    def deck(self, bench: str) -> str:
        ...
        `.control` block (`print`/`meas` scalars for `lab.metrics.promote`, and/or `write sim.raw`).
