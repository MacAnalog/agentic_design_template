...
| deck variables | `design.sim.DECK_VARS` — the machine-specific paths a deck names as `$VAR` and `sim.run` resolves at the moment of simulating (a model library installed only on some machines). The deck text, the ledger row and the frozen reference stay portable; `deck-portable` in `scripts/lint.py` refuses an absolute include in a frozen deck. Pin WHICH library by revision name in the PDK row above — never its path |
