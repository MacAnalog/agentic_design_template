...
| simulator lane | `lane:` in `harness.yaml` picks it, and `design/sim.py` re-exports that module. **Absent = `ngspice`**, the open lane: native ngspice through `design/sim_ngspice.py`, this repo's policy over the platform's `spicexplorer_core.spice_engine.run_deck`. The binary is `$<sim_env>`, defaulted by the `<PREFIX>` rule (`FOO_EXP` → `FOO_NGSPICE`; `SIM_NGSPICE` when `exp_env` has no prefix), else `ngspice` on PATH |
| bridge lane | `lane: bridge` → `design/sim_bridge.py`: a commercial kit whose models load only in the vendor simulator on the lab's EDA server. The deck goes there through the lab's bridge and the results come back; the reusable half is the platform's bridge-lane package, which this repo only WRAPS (work root, `$<sim_env>` launcher, `MODE`, the doctor). `make doctor` reports exit 2 — not a stop — when this machine has no bridge profile |
| model library (bridge lane) | `design/pdk.py` resolves it from the environment, in order: `$<DESIGNTAG>_PDK_LIB` (scoped to THIS design — what a person should export), `$<PREFIX>_PDK_LIB` (the name the frozen decks carry as `$`-token text), then the per-MACHINE `$<PDK-ID>_PDK_LIB` derived from `pdk:` in `harness.yaml` (`ihp-sg13g2` → `IHP_SG13G2_PDK_LIB`), which belongs in the lab's per-account env file beside `SX_ROOT` and serves every design in that process. **Nothing is scanned for**: the path is per machine, so it comes from the environment and not from a neighbouring clone's committed deck — that route depended on which repos a person had cloned and broke inside a git worktree. Every route must carry `REVISION` or it is refused (`<NAME>_ALLOW_MISMATCH=1` for a typed one-off); no message ever echoes the value |
...
| scratch budget | `$SX_SCRATCH_WARN_GB` (default 50) is the size this checkout's work dir may reach before `make doctor` and `make lint` say so — the doctor prints the usage on every run, `scratch_budget` in `scripts/lint.py` warns (never fails) when the biggest run dirs over that mark have no reduction row. A raw record is scratch, not evidence: reduce it, commit the reduction, then `make clean-runs` (`AGE=<hours>`, default 24; `ARGS="--dry-run"` plans only). `make clean` remains the coarse one — the whole work dir |
...
| doctor | `make doctor` (`python -m design.sim`, dispatched to the selected lane) simulates a one-resistor deck and requires `i_ma = 1` parsed from the log, a rawfile and the per-run init marker; it fails when `SPICE_USERINIT_DIR` is unset or its `.spiceinit` is empty. Once the design has a PDK device, call `preflight(deck, expect)` with its own probe. On the bridge lane it reports an unset model-library variable WITHOUT simulating: the probe is one resistor and would pass without the kit, but a lane that cannot bind this design to its library is not alive |
